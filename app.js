...
function submitQuiz() {
  ...
  // Antwoorden van de juiste antwoorden voor de vragen
  const correctAnswers = {
    ...
    q6: "b", // Antwoord voor vraag 6 is B (Moslims zien geweld als gerechtvaardigd in bepaalde gevallen)
  ...
  for (let [question, answer] of formData.entries()) {
    if (correctAnswers[question] === answer) {
      ...
      // Markeer de vraag als correct (zonder invoervelden uit te schakelen)
      const inputs = document.getElementsByName(question);
      inputs.forEach(input => {
        if (input.value === answer) {
          input.classList.add("correct"); // Voeg een visuele indicatie toe
        }
      });
    }
  }
